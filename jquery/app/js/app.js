...
jQuery(function ($) {
    ...
    reloadDB: function (prefix) {
      this.prefix = 'todos-jquery' + ('-' + prefix) || '';
      var self = this;
      ...
      if (prefix) {
        var data = firebase.database().ref('todos/' + this.prefix).once('value').then(function(snapshot) {
          //return snapshot.val();
          if (snapshot.val()) {
            util.store(self.prefix, JSON.parse(snapshot.val().data));
          } else {
            self.todos = [];
            util.store(self.prefix, []);
          }
          self.render(true);
        });
      }
    ...
    firebase.auth().onAuthStateChanged(function(user) {
      if (user) {
        ...
        user.getToken().then(function(accessToken) {
          ...
          App.loginBtn.text('Logout');
        });
      } else {
        // User is signed out.
        App.isUser = false;
        App.reloadDB(); // reload DB from localStorage
        App.loginBtn.text('Login?');
      }
    }, function(error) {
      console.log(error);
    });
